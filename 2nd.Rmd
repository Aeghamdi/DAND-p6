---
title: "Explore and Summarize Data"
author: "Abdulelah Alghamdi"
date: "January 18, 2019"
output: 
 html_document:
    fig_caption: yes
    keep_md: yes
    theme: readable
    toc: yes
    
---
##The chosen data set

White Wine Quality.  


```{r echo=FALSE, message=FALSE, warning=FALSE, packages}
#Importing the needed librariees
library(ggplot2)
library(dplyr)
library(tidyr)
library(corrplot)
```


# Univariate Plots Section

```{r echo=FALSE, cache=TRUE, DataSet}
#reading the dataset from the source file
white_wines <- read.csv("C:/Users/Powerfull/Desktop/new_R_python/wineQualityWhites.csv",  row.names = 1, stringsAsFactors = FALSE)
white_wines$quality <- factor(white_wines$quality, ordered = TRUE)
```

```{r echo=FALSE, Names}
#showing the coulmns names
names(white_wines)
glimpse(white_wines)
```

All variables are numerical except for the quality which is represented as a ordered factor.

## Quality

```{r echo=FALSE, quality}
#the summary of the quality to see the data type and other features
summary(white_wines$quality)
#bar plot for the quality variable
ggplot(data = white_wines, aes(x = quality))+
  geom_bar(color="black", fill="skyblue")+
  ggtitle("Distribution of Quality")
```

The Quality is normally distributed.  
The minimum is 3 and the maximum is 9. 

We will create new variable to represents the Quality level.  
I will call it quality_level. with the levels given below.

* 0 - 4 : low
* 5 - 6 : good
* 7 - 10 : perfect

```{r echo=FALSE, quality_level}
#creating new variable
white_wines$quality_level <- ifelse(white_wines$quality < 5, 'low', ifelse(
white_wines$quality < 7, 'good', 'perfect'))
white_wines$quality_level <- ordered(white_wines$quality_level, levels = c('low', 'good', 'perfect'))


#the summary of the quality level to see the data type and other features
summary(white_wines$quality_level)
#bar plot for the quality level variable
ggplot(data = white_wines, aes(x = as.factor(quality_level), fill = quality_level)) +
  geom_bar() +
  theme_minimal()+  scale_fill_brewer()+
  xlab("Quality Level") +
  ggtitle("Distribution of Quality Level")
```


## Fixed Acidity

```{r echo=FALSE, fixed_Acidity}
#the summary of the Fixed Acidity to see the data type and other features
summary(white_wines$fixed.acidity)
#bar plot for the Fixed Acidity variable
ggplot(data = white_wines, aes(x = fixed.acidity))+
  geom_histogram(color="black", fill="skyblue",binwidth=0.2)+
  xlab("Fixed Acidity (g/dm^3)") +
  ggtitle("Distribution of Fixed Acidity")
```

The Fixed Acidity is normaly distributed.  
The max is 14.2 $g/dm^3$. and the min is 3.8 $g/dm^3$ and median is 6.8 $g/dm^3$.
There are some outliers.

## Volatile acidity

```{r echo=FALSE, Volatile_Acidity}
#the summary of the Volatile Acidity to see the data type and other features
summary(white_wines$volatile.acidity)
#bar plot for the Volatile Acidity variable
ggplot(data = white_wines, aes(x = volatile.acidity))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.02)+
  xlab("Volatile Acidity (g/dm^3)") +
  ggtitle("Distribution of Volatile Acidity")
```

The Volatile Acidity is normaly distributed.  
The max is 1.1 $g/dm^3$ and the min is 0.08 $g/dm^3$ and median is 0.026 $g/dm^3$  
There are some outliers.

## Citric acid

```{r echo=FALSE, Citric_Acid}
#the summary of the Citric acid to see the data type and other features
summary(white_wines$citric.acid)
#bar plot for the Citric acid variable
ggplot(data = white_wines, aes(x = citric.acid))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.025)+
  xlab("Citric Acid (g/dm^3)") +
  ggtitle("Distribution of Citric Acid")
```

The Citric Acidity is normaly distributed.  
The max is 1.66 and the min is 0 $g/dm^3$ and median is 0.32 $g/dm^3$.   
There are some outliers.

## Residual sugar

```{r echo=FALSE, Residual_Sugar}
#the summary of the Residual sugar to see the data type and other features
summary(white_wines$residual.sugar)
#bar plot for the Residual sugar variable
ggplot(data = white_wines, aes(x = residual.sugar))+
  geom_histogram(color="black", fill="skyblue",binwidth =1)+
  xlab("Residual Sugar (g/dm^3)") +
  ggtitle("Distribution of Residual Sugar")
```

The Residual sugar is right skewed distributed.  
The max is 65.8 and the min is 0.6 $g/dm^3$ and median is 5.2 $g/dm^3$.   
There are some outliers.

## Chlorides

```{r echo=FALSE, Chlorides}
#the summary of the Chlorides to see the data type and other features
summary(white_wines$chlorides)
#bar plot for the Chlorides variable
ggplot(data = white_wines, aes(x = chlorides))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.005)+
  xlab("Chlorides (g/dm^3)") +
  ggtitle("Distribution of Chlorides")
```

The max is 0.346 $g/dm^3$ and the min is 0.009 $g/dm^3$ and median is 0.043 $g/dm^3$.  
There are some outliers.

## Free sulfur dioxide

```{r echo=FALSE, Free_Sulfur_Dioxide}
#the summary of the Free sulfur dioxide to see the data type and other features
summary(white_wines$free.sulfur.dioxide)
#bar plot for the Free sulfur dioxide variable
ggplot(data = white_wines, aes(x = free.sulfur.dioxide))+
  geom_histogram(color="black", fill="skyblue",binwidth =5)+
  xlab("Free sulfur dioxide (mg/dm^3)") +
  ggtitle("Distribution of Free sulfur dioxide")
```

The Free sulfur dioxide is normally distributed.  
The max is 289 $mg/dm^3$ and the min is 2 $mg/dm^3$ and median is 34 $mg/dm^3$.  
There are some outliers.

## Total sulfur dioxide

```{r echo=FALSE, total_sulfur_dioxide}
#the summary of the Total sulfur dioxide to see the data type and other features
summary(white_wines$total.sulfur.dioxide)
#bar plot for the Total sulfur dioxide variable
ggplot(data = white_wines, aes(x = total.sulfur.dioxide))+
  geom_histogram(color="black", fill="skyblue",binwidth =10)+
  xlab("Total sulfur dioxide (mg/dm^3)") +
  ggtitle("Distribution of Total sulfur dioxide")
```

The Total sulfur dioxide is normally distributed.  
The max is 440 $mg/dm^3$ and the min is 9 $mg/dm^3$ and median is 134 $mg/dm^3$.    
There are some outliers.

## Density

```{r echo=FALSE, Density}
#the summary of the Density to see the data type and other features
summary(white_wines$density)
#bar plot for the Total sulfur dioxide variable
ggplot(data = white_wines, aes(x = density))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.001)+
  xlab("Density (g/cm^3)") +
  ggtitle("Distribution of Density")
```

The Density is normally distributed.  
The max is 1.039 $mg/cm^3$ and the min is 0.9871 $mg/cm^3$ and median is 0.9937 $mg/cm^3$     
There are some outliers.

## pH

```{r echo=FALSE, PH}
#the summary of the pH to see the data type and other features
summary(white_wines$pH)
#bar plot for the pH variable
ggplot(data = white_wines, aes(x = pH))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.02)+
  xlab("pH") +
  ggtitle("Distribution of pH")
```

The pH is normally distributed.  
The max is 3.82 and the min is 2.72 and median is 3.18     
There are some outliers.


## Sulphates

```{r echo=FALSE, Sulphates}
#the summary of the Sulphates to see the data type and other features
summary(white_wines$sulphates)
#bar plot for the Sulphates variable
ggplot(data = white_wines, aes(x = sulphates))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.02)+
  xlab("sulphates (g / dm3)") +
  ggtitle("Distribution of sulphates")
```

The Sulphates is normally distributed.  
The max is 1.08 $g/dm^3$ and the min is 0.22 $g/dm^3$ and median is 0.47 $g/dm^3$.  
There are some outliers.

## Alcohol

```{r echo=FALSE, Alcohol}
#the summary of the Alcohol to see the data type and other features
summary(white_wines$alcohol)
#bar plot for the Alcohol variable
ggplot(data = white_wines, aes(x = alcohol))+
  geom_histogram(color="black", fill="skyblue",binwidth =0.2)+
  xlab("Alcohol %") +
  ggtitle("Distribution of Alcohol")
```

The max is 14.2% and the min is 8% and median is 10.4%.    
There are some outliers.

# Univariate Analysis


### What is the structure of your dataset?

The dataset is about the White Wine and it has 12 variables and 4,898 observations.

### What is/are the main feature(s) of interest in your dataset?

It can not be anything but the Quality.   
That's why we have to figure out what can affect the quality in a positive or a negative way.

### What other features in the dataset do you think will help support your investigation into your feature(s) of interest?

The number of variables is 12 which is not a small number so investigating it will lead to a really surprising and lots of insights in my opinion.  

### Did you create any new variables from existing variables in the dataset?

yes, I created 1 variable it's called "Quality Level" is used to describe the quality rate ( low, good, perfect).  

### Of the features you investigated, were there any unusual distributions? Did you perform any operations on the data to tidy, adjust, or change the form of the data? If so, why did you do this?

There were no unusual distributions.  
just some outliers which i think its better to keep it, because it will be useful for the data to be realistc that will lead the analysis of the relationships between the variables to be more accurate, that's why I will not remove these outliers.  
No missing values and no need to adjust the data.  


# Bivariate Plots Section


##correlation matrix

In order to start the Bivariate section we will begin with the followig correlation matrix plot to visualize the relationship between the variables:

```{r echo=FALSE,Correlation_Matrix}
#creating the correlation matrix
var1 <-cor(subset(white_wines,select=-c(quality,quality_level)),method = "pearson")
corrplot(var1)
```

Since we will mainly focus on the Quality and its relationship with the rest of the variables, we will visualize each variable with the quality in the below plots.  

## Fixed Acidity vs. Quality

```{r echo=FALSE, Fixed_Acidity_vs_Quality11}
#creating plot for 2 variables (Fixed Acidity vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), fixed.acidity)) + geom_boxplot()+ylab("Fixed Acidity (g / dm^3)") +xlab("Quality")+
  ggtitle("Fixed Acidity VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span = 0.5)
```

It has a small negative effect on the white wine quality.  


## Volatile Acidity vs. Quality

```{r echo=FALSE, Volatile_Acidity_vs_Quality11}
#creating plot for 2 variables (Volatile Acidity vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), volatile.acidity)) + geom_boxplot()+ylab("Volatile Acidity (g / dm^3)") +xlab("Quality")+
  ggtitle("Volatile Acidity VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span = 0.5)
```

We have to consider the Volatile Acidity as a negative factor to the white wine quality.  
The relationship between them is an Inverse relationship.

## Citric Acid vs. Quality


```{r echo=FALSE, Citric_Acid_vs_Quality11}
#creating plot for 2 variables (Citric Acid vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), citric.acid)) + geom_boxplot()+ylab("Citric Acid (g / dm^3)") +xlab("Quality")+
  ggtitle("Citric Acid VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span = 0.5)
```

If you focus on the line it shows that there is no relationship between the quality and citric acid at all.  


## Residual sugar vs. Quality


```{r echo=FALSE, Residual_sugar_vs_Quality11}
#creating plot for 2 variables (Residual sugar vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), residual.sugar)) + geom_boxplot()+ylab("Residual sugar (g / dm^3)") +xlab("Quality")+
  ggtitle("Residual sugar VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span = 0.5)
```

The Residual sugar is effecting the white wine quality in a negative way but not major affect so will look for other variables with stronger affect.


## Chlorides vs. Quality


```{r echo=FALSE, chlorides_vs_Quality11}
#creating plot for 2 variables (Chlorides vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), chlorides)) + geom_boxplot()+ylab("Chlorides") +xlab("Quality")+
  ggtitle("Chlorides VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span =0.5)
```

Chlorides is effecting the with wine quality in a negative way even though the range of the used amount of Chlorides is too small, the max is 0.346 and min is 0.009.  

## Free sulfur dioxide vs. Quality


```{r echo=FALSE, Free_sulfur_dioxide_vs_Quality11}
#creating plot for 2 variables (Free sulfur dioxide vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), free.sulfur.dioxide)) + geom_boxplot()+ylab("Free sulfur dioxide (mg / dm^3)") +xlab("Quality")+
  ggtitle("Free sulfur dioxide VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span =0.5)
```

No relationship with the white wine quality at all.


## Total sulfur dioxide vs. Quality


```{r echo=FALSE, Total_sulfur_dioxide_vs_Quality11}
#creating plot for 2 variables (Total sulfur dioxide vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), total.sulfur.dioxide)) + geom_boxplot()+ylab("Total sulfur dioxide (mg / dm^3)") +xlab("Quality")+
  ggtitle("Total sulfur dioxide VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span =0.5)
```

Total sulfur dioxide is affecting the quality in a negative way as it's shown in the plot above.


## Density vs. Quality


```{r echo=FALSE, Density_vs_Quality11}
#creating plot for 2 variables (Density vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), density)) + geom_boxplot()+ylab("Density (g/cm^3)") +xlab("Quality")+
  ggtitle("Density VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span =0.5)
```

Obviously, it is an inverse relationship, the density reaches the lowest point with the perfect white wine quality.


## PH vs. Quality


```{r echo=FALSE, PH_vs_Quality11}
#creating plot for 2 variables (PH vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), pH)) + geom_boxplot()+ylab("PH") +xlab("Quality")+
  ggtitle("PH VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span =0.5)
```

Positive relationship, the More you use the PH, the more white wine quality you get.


## Sulphates vs. Quality


```{r echo=FALSE, Sulphates_vs_Quality11}
#creating plot for 2 variables (Sulphates vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), sulphates)) + geom_boxplot()+ylab("Sulphates (g / dm3)") +xlab("Quality")+
  ggtitle("Sulphates VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span =0.5)
```

The plot shows clearly that the sulphates is having a positive relationship with the white wine quality but not a strong one.


## Alcohol vs. Quality


```{r echo=FALSE, alcohol_vs_Quality11}
#creating plot for 2 variables (Alcohol vs. Quality)
varr <- ggplot(white_wines, aes(factor(quality), alcohol)) + geom_boxplot()+ylab("Alcohol %") +xlab("Quality")+
  ggtitle("Alcohol VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span = 0.5)
```

Its very obvious that alcohol variable is playing a big role in the white wine quality.  
Alcohol had strongest Positive relationship with the quality in the hole dataset.


# Bivariate Analysis

#### Talk about some of the relationships you observed in this part of the investigation. How did the feature(s) of interest vary with other features in the dataset?

I investigated and visulized in this part above in order to know the affect of all the variables of the dataset on the quality of the white wine, in order to choose the most important variables that will be focused on to complete this project.  

Variables with Positive relationships are: PH, Sulphates and Alcohol.  
Variables with inversal relationships are: Fixed Acidity, Volatile Acidity, Residual sugar, Chlorides and Total sulfur dioxide.  

#### Did you observe any interesting relationships between the other features (not the main feature(s) of interest)?

I did not excpect that the inversal relationships will be more than the positive relationships, but it was the opposite.  

I can say now that not all variables of any dataset should affect (positivly or negativly) on a particural variable in the same dataset.  

#### What was the strongest relationship you found?

Clearly its the alcohol with the white wine quality.


# Multivariate Plots Section

After the past section we have some ideas about the most affecting variables that we will continue our project investigating it.  

##PH vs Quality
```{r echo=FALSE, PH_QUALITY}
#Multi plot to compaire the effect of pH and Quality based on the Quality Level
ggplot(white_wines, aes(x=quality, y=pH, color=quality_level))+
  geom_point(alpha = 1/3)+
  ylab("pH") +xlab("Quality")+
  ggtitle("pH VS Quality")+
  geom_smooth(se=FALSE, method="lm")+theme_dark()+
  scale_color_brewer(type='qual', guide=guide_legend(title='Quality Level'))
```


##Alcohol vs Quality


```{r echo=FALSE, ALCOHOL_QUALITY}
#Multi plot to compaire the effect of Alcohol and Quality based on the Quality Level
ggplot(white_wines, aes(x=quality, y=alcohol, color=quality_level))+
  geom_point(alpha = 1/3)+
  ylab("alcohol %")+xlab("Quality")+
  ggtitle("alcohol VS Quality")+
  geom_smooth(se=FALSE, method="lm")+theme_dark()+
  scale_color_brewer(type='qual', guide=guide_legend(title='Quality Level'))
```

##Total sulfur dioxide vs Quality


```{r echo=FALSE, Total_sulfur_dioxide_QUALITY}
#Multi plot to compaire the effect of Total sulfur dioxide and Quality based on the Quality Level
ggplot(white_wines, aes(x=quality, y=total.sulfur.dioxide, color=quality_level))+
  geom_point(alpha = 1/3)+
  ylab("Total sulfur dioxide (mg/dm^3)")+xlab("Quality")+
  ggtitle("Total sulfur dioxide VS Quality")+
  geom_smooth(se=FALSE, method="lm")+theme_dark()+
  scale_color_brewer(type='qual', guide=guide_legend(title='Quality Level'))
```


##Density vs Quality


```{r echo=FALSE, Density_QUALITY}
#Multi plot to compaire the effect of Density and Quality based on the Quality Level
ggplot(white_wines, aes(x=quality, y=density, color=quality_level))+
  geom_point(alpha = 1/3)+
  ylab("Density (g/cm^3)")+xlab("Quality")+
  ggtitle("Density VS Quality")+
  geom_smooth(se=FALSE, method="lm")+theme_dark()+
  scale_color_brewer(type='qual', guide=guide_legend(title='Quality Level'))
```

##Volatile Acidity vs Quality

```{r echo=FALSE, Volatile_Acidity_QUALITY}
#Multi plot to compaire the effect of Volatile Acidity and Quality based on the Quality Level
ggplot(white_wines, aes(x=quality, y=volatile.acidity, color=quality_level))+
  geom_point(alpha = 1/3)+
  ylab("Volatile Acidity (g/dm^3)")+xlab("Quality")+
  ggtitle("Volatile Acidity VS Quality")+
  geom_smooth(se=FALSE, method="lm")+theme_dark()+
  scale_color_brewer(type='qual', guide=guide_legend(title='Quality Level'))
```

##volatile Acidity vs Alcohol based on Quality Level

We know that alcohol is affecting positively and Volatile acidity is affecting negatively so we will involve both of it based on the quality level in the below plot:

```{r echo=FALSE, vars}
#volatile Acidity vs Alcohol based on Quality Level plot
ggplot(white_wines, aes(x=alcohol, y=volatile.acidity, color=quality_level))+
  geom_point()+
  ylab("Volatile Acidity (g/dm^3)")+xlab("Alcohol")+
  ggtitle("Volatile Acidity VS Alcohol")+theme_dark()+
  scale_color_brewer(type='div', guide=guide_legend(title='Quality Level'))
```

The majority of the low quality white wine(colored in broun) are having small amount of alcohol and having large amount of volatile Acidity.That why most of those points are gathering in the left-top corner.  
while the majority of the pefect quality white wine(colored in green) are having large amount of alcohol and having small amount of volatile Acidity. That why most of those points are gathering in the right-bottom corner.  

# Multivariate Analysis


### Talk about some of the relationships you observed in this part of the investigation. Were there features that strengthened each other in terms of looking at your feature(s) of interest?

We visualized the strongest variables in the dataset and used multi variables plots.  
We made sure that the alcohol effecting the white wine quality positively while th volatile acidity and Total sulfur dioxide are affecting the white wine quality negativly.

### Were there any interesting or surprising interactions between features?

No 

### OPTIONAL: Did you create any models with your dataset? Discuss the strengths and limitations of your model.

No

### Were there any interesting or surprising interactions between features?


### OPTIONAL: Did you create any models with your dataset? Discuss the 
No

# Final Plots and Summary



### Plot One


```{r echo=FALSE, finalquality}
ggplot(data = white_wines, aes(x = quality))+
  geom_bar(color="black", fill="skyblue")+
  ggtitle("Distribution of Quality")
```

### Description One

The above plot represents the nature and distribution of the most important variable in our dataset which is the quality.  
This helped us to create the new variable ( Quality Level) and helped us to build our plan to invistigate and analys the variables that can play major role in increasing the quality of the white wine and how it can be obtained.

### Plot Two


```{r echo=FALSE, alcohol_vs_Qualityfinal}
varr <- ggplot(white_wines, aes(factor(quality), alcohol)) + geom_boxplot()+ylab("Alcohol %") +xlab("Quality")+
  ggtitle("Alcohol VS Quality")
varr + geom_smooth(method = "lm", se=TRUE, aes(group=1),span = 0.5)
```

### Description Two

The above plot is one of the main keys in our analysis process because it represents the strongest relationship between the quality and the other variables in our data set.  

While i dont have any idea about the wine and i never tried it, this plot have given me an important insight which is the quality of the white wine is mostly related to the amount of alcohol, this can help me if i wanted to bring a bottle of wine to a friend as gift or trying to order it in some restaurant in the future.


### Plot Three

```{r echo=FALSE, varsfinal}
ggplot(white_wines, aes(x=alcohol, y=volatile.acidity, color=quality_level))+
  geom_point()+
  ylab("Volatile Acidity (g/dm^3)")+xlab("Alcohol %")+
  ggtitle("Volatile Acidity VS Alcohol")+theme_dark()+
  scale_color_brewer(type='div', guide=guide_legend(title='Quality Level'))
```

### Description Three

Really useful and clear insights can be taken from this plot.  

It contains one positive variable(alcohol) and another negative variable(volatile acidity) and shows you how thes variables can affect the quality level which is involved too by being represented as the colors  of the plot's shapes.  

The majority of the low quality white wine(colored in broun) are having small amount of alcohol and having large amount of volatile Acidity.That why most of those points are gathering in the left-top corner.  

while the majority of the pefect quality white wine(colored in green) are having large amount of alcohol and having small amount of volatile Acidity. That why most of those points are gathering in the right-bottom corner. 


------

# Reflection

The chosen data set(White Wine Quality) is suitable to perform analysis on the quality of the wine and how to maintain it, the data set contains most of the components that being used in the white wine.  

I am quite surprised because the used amount of the water was not exist in the data set.  

Maybe its related to the density so the more water you use, the less density you get, not quite sure but i think if the water amount was there it will be perfect data set and the analysis would be more useful even for wine manifactoring purposes.  

The dataset contains 4,898 observations with 12 variables.  

I started by exploring each variable individually and looking at the distribution of each one along with a simple comment/insight below the plot.  

There are 11 chemical variables for each observation in the wine dataset which outputs a quality number.  

I used the quality variable to create a categorical factor(new variable) called Quality Level, it devides the quality to three levels low, good and perfect.  

Most important insights is that white wine quality was highly and positively correlated with alcohol.  

Secondly, white wine quality depends on volatile acidity and Total sulfur dioxide but negatively, which means using high levels of volatile acidity or Total sulfur dioxide can lead your wine quality to be low.  

I found this project very important maybe the hardest project in the DAND course so far, it requires knowledge of the R syntax and plot types and plot parameters and which data to use in the same plot and how to visulize it in a proper way to help figuring the insights easly.

#References 

https://ggplot2.tidyverse.org/reference/geom_histogram.html

https://www.rdocumentation.org/packages/tibble/versions/1.4.2/topics/rownames

https://stackoverflow.com/questions/46128392/warning-when-building-a-function-in-r

https://ggplot2.tidyverse.org/reference/geom_bar.html

https://stackoverflow.com/questions/38839923/r-ggplot-bar-plot-with-month-on-x-axis

https://ggplot2.tidyverse.org/reference/

https://www.r-bloggers.com/how-to-expand-color-palette-with-ggplot-and-rcolorbrewer/

https://stackoverflow.com/questions/10438752/adding-x-and-y-axis-labels-in-ggplot2

https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/colorPaletteCheatsheet.pdf

https://ggplot2.tidyverse.org/reference/geom_smooth.html

https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html

https://ggplot2.tidyverse.org/reference/ggtheme.html

http://ggplot.yhathq.com/docs/scale_color_brewer.html

https://ggplot2.tidyverse.org/reference/geom_point.html

https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf



